...
def get_live_nepse_data(symbol="NABIL"):
    try:
        ...
        print(f"Error fetching live NEPSE data: {e}")
...
def generate_pl_analysis(current_price, predictions):
    """Compares today's price to predicted closing prices to determine Profit/Loss."""
    pl_data = []
...
def home():
    ...
    <head>
        ...
        <style>
            :root {
                --bg-deep: #0a3d91; --bg-light: #1a5cb5; --text-main: #ffffff; --text-muted: #d0dcf0;
                --up-color: #4ade80; --down-color: #f87171; --card-bg: rgba(255, 255, 255, 0.1); --border: rgba(255, 255, 255, 0.2);
            }
            * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Inter', sans-serif; }
            ...
            .stat-card h2 { font-size: 18px; color: var(--text-muted); margin-bottom: 10px; }
            ...
            .table-section h2 { margin-bottom: 20px; font-size: 24px; }
            ...
            .status-profit { color: var(--up-color); font-weight: bold; }
            .status-loss { color: var(--down-color); font-weight: bold; }
        </style>
    </head>
    <body>
        <header>
        ...
        <div class="accuracy-banner">
            <h3>{{ accuracy }}%</h3>
            <p>AI Accuracy Report - Last 24 Days (NABIL Bank)</p>
        </div>
        ...
        <script>
            // 1. Load Backtesting Data (Past)
            fetch('/api/backtest')
                .then(response => response.json())
                .then(data => {
                    const tableBody = document.getElementById('backtestTable');
                    data.forEach(row => {
            ...
            // 2. Load Profit/Loss Analysis
            fetch('/api/pl_analysis')
                .then(response => response.json())
                .then(data => {
            ...
            // 3. Load Forward Predictions (Future)
            fetch('/api/predict')
                .then(response => response.json())
                .then(data => {
                ...
                .catch(error => console.error('Error fetching forward data:', error));
        </script>
    </body>
    </html>
    '''
...
def get_pl_analysis():
    ...
    pl_data = generate_pl_analysis(live_data['price'], predictions)
